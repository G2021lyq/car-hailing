...
int main(int argc, char* argv[])
{
	...
	*/
	...
	char data0[2048] = { "hello zengming 123666" };
	if (argc == 1) {
		cout << "请输入文本或网址信息（按回车结束）：";
		gets_s(data);
	}
	else {
		sprintf(data0, "%s/%s", argv[1], argv[2]);
		std::string ipv4Address = GetLocalIPv4Address();
		sprintf(data, "http://%s:8080/payment/%s", ipv4Address.c_str(), data0);
	}
